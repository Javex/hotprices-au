...
def test_get_canonical():
    ...
    item = get_item(
        description="MINI CHRISTMAS CARD 20PK", quantity=0, isWeighted=False
    )
    can_item = coles.get_canonical(item, today)
    assert can_item["unit"] == "ea"
    ...
    item = get_item(
        description="BEER CAN 375ML:PACK6", quantity=0, isWeighted=False, size=""
    )
    can_item = coles.get_canonical(item, today)
    ...
    item = get_item(
        description="CARLTON DRY 8PK CAN 375ML:CTN24",
        quantity=0,
        isWeighted=False,
        size="",
    )
    ...
    item = get_item(
        description="MOON DOG FIZZER SELTZER 6% CAN 330ML (10PK):CTN30",
        quantity=0,
        isWeighted=False,
        size="",
    )
    ...
    item = get_item(
        description="HARD SOLO CAN 375ML 10PK:CTN30",
        quantity=0,
        isWeighted=False,
        size="",
    )
    ...
    item = get_item(
        description="SMIRNOFF RED 37.5% VODKA 375ML:EA",
        quantity=0,
        isWeighted=False,
        size="",
    )
